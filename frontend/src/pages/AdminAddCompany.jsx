...
function AdminAddCompany() {
  ...
  const uploadCompanyImages = async () => {
    ...
    const response = await fetch("/api/suppliers/upload-images", {
      method: "POST",
      body: uploadData,
    });
    ...
    if (!response.ok || !data.success) {
      throw new Error(data.message || "Failed to upload company images");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Upload images first
      const { logoUrl, galleryImages } = await uploadCompanyImages();
      ...
      alert(error.message || "Unable to create company");
    } finally {
      setSubmitting(false);
    }
  };
                ...
                <div>
                  ...
                  {galleryPreviews.length > 0 && (
                    <p className="mt-2 text-[11px] text-slate-400">
                      ...
                      {galleryPreviews.length !== 1 ? "s" : ""} selected
                    </p>
                  )}
                </div>
              </div>
